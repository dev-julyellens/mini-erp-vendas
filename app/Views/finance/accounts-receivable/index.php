...
$subtitle = 'Geradas automaticamente a cada venda aprovada';
$breadcrumbs = [
...
<div class="col-6 col-md-2">
    <label class="form-label" for="filter_due_from">Venc. de</label>
    <input type="date" class="form-control" id="filter_due_from" name="due_from"
        value="<?= htmlspecialchars($filters['due_from'], ENT_QUOTES, 'UTF-8') ?>">
</div>
<div class="col-6 col-md-2">
    <label class="form-label" for="filter_due_to">Venc. até</label>
    <input type="date" class="form-control" id="filter_due_to" name="due_to"
        value="<?= htmlspecialchars($filters['due_to'], ENT_QUOTES, 'UTF-8') ?>">
</div>
<div class="col-12 col-md-3 d-flex align-items-end">
    <div class="form-check mb-2">
        <input class="form-check-input" type="checkbox" id="filter_overdue" name="overdue" value="1"
            <?= $filters['overdue'] === '1' ? 'checked' : '' ?>>
        <label class="form-check-label" for="filter_overdue">Somente vencidas</label>
    </div>
</div>
<?php
$filterContent = ob_get_clean();
$filterAction = $url('finance/accounts-receivable');
$filterClearHref = $url('finance/accounts-receivable');
                    ...
                    <?php foreach ($accounts as $ar): ?>
                        ...
                        <tr class="<?= $ar->isOverdue() ? 'table-warning' : '' ?>">
                            ...
                            <td class="text-end">
                                ...
                                $canEdit = false;
                                $canDelete = false;
                                $detailsHref = $url('finance/accounts-receivable/show?id=' . $ar->id);
                                $extraActions = [];
                                if ($canReceive && $ar->canReceive())
                                {
                                    $extraActions[] = [
                                        'href' => $url('finance/accounts-receivable/receive?id=' . $ar->id),
                                        'label' => 'Receber',
                                        'variant' => 'primary',
                                    ];
                                }
